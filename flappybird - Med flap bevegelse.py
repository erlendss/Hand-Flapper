...
def main():
    ...
    while not done:
        ...
        if board.analog[0].read() is None:
            ...
            value_H = float(board.analog[0].read())
            value_L = float(board.analog[1].read())                       
            if (value_H < AVG_H*FUDGE) and (value_L < AVG_L*FUDGE):
                if CAN_FLAP:
                    bird.msec_to_climb = Bird.CLIMB_DURATION
                    CAN_FLAP = False  
            if (value_H > AVG_H*FUDGE) and (value_L > AVG_L*FUDGE):
                CAN_FLAP = True
            if (value_H < MIN_H*1.1):
                paused = True
                print "Du var naer hoyre kopp! FAIL"
            if (value_L < MIN_L*1.1):
                paused = True
                print "Du var naer venstre kopp! FAIL"
        

        if paused:
        
            done_surface = score_font.render("FAIL", True, (255, 255, 255))
            done_x = WIN_WIDTH/2 - score_surface.get_width()/2
            display_surface.blit(done_surface, (done_x, PipePair.PIECE_HEIGHT+100))
            pygame.display.flip()
